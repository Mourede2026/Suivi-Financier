...
### Côté administrateur
- **Équipes & accès** : créer une équipe (nom + identifiant), lui attribuer
  un ou plusieurs centres de santé via des cases à cocher, générer ou définir
  son mot de passe. Une équipe ne peut se connecter qu'avec ses identifiants
  et ne verra jamais les centres qui ne lui sont pas affectés (le backend
  applique ce contrôle à chaque requête, pas seulement l'interface).
- **Vue d'ensemble** : tableau de bord global, filtrable par commune ou par
  ...
  par centre.
- **Centres de santé** : liste de référence des centres.

### Côté équipe
- **Recettes** / **Dépenses** : formulaire de saisie mensuelle (par centre,
  parmi ceux qui lui sont affectés), avec calcul automatique des écarts et
  historique modifiable.
- **Mon tableau de bord** : mêmes analyses que l'admin, mais restreintes aux
  seuls centres affectés à l'équipe connectée.
- **Mes centres** : rappel des centres affectés.
